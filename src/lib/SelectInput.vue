<template>
   <div 
      ...
      :class="[customClass, {'focused': isFocused, [colorClasses.ring]: isFocused}]"
      @focus="handleFocus"
      @blur="handleBlur"
      tabindex="0"
   >
     ...
     <div class="select-input" @click="toggleDropdown($event)">
       <template v-if="multiple && selectedOptions.length">
         <div class="selected-badges">
            <span v-for="option in selectedOptions" :key="option[idKey]" :class="['badge', colorClasses.bg, colorClasses.text]">
               {{ option[labelKey] }}
               <button @click.stop="removeOption(option)" :class="['remove-badge', colorClasses.text, colorClasses.hoverText]">
       ...
       <div class="option-list">
         <template v-for="(item, index) in filteredOptions" :key="index">
            <div
               ...
               :class="['option', `indent-${item.level}`, { 'selected': isSelected(item), [colorClasses.bg]: isSelected(item), [colorClasses.text]: isSelected(item) }]"
     ...
     <div :class="['caret-icon', {[colorClasses.caretText]: isFocused}]">
   ...
   computed: {
      ...
      colorClasses() {
         return {
            ...
            bg: `bg-${this.mainColor}-100`,
            text: `text-${this.mainColor}-800`,
            hoverText: `hover:text-${this.mainColor}-800`,
            caretText: `text-${this.mainColor}-500`,
         }
      },
   },
   watch: {
...
.badge {
  @apply inline-flex items-center px-2 py-0.5 rounded-md text-sm;
}
.remove-badge {
  @apply ml-1 p-0.5 rounded-full hover:bg-opacity-20 transition-colors duration-200;
}
.remove-badge svg {
  @apply w-3 h-3;
}
